...
class DecisionMatrix:
    def __init__(self, root):
        ...
        self.criteria = ['Cost & Material Efficiency', 'Building Envelope Efficiency', 'Constructability', 'Functionality/Flexibility', 'Aesthetics']
    ...
    def calc_plot(self):
        ...
        for alt in self.alternatives:
            ...
            for c in self.criteria:
                try:
                    val = float(self.score_entries[(alt, c)].get())
                    alt_vals[alt].append(val)
                except ValueError:
        ...
        # Cost & Building Envelope → lower better
        for i in range(2):
            # find lowest among alternatives
            lowest = min([alt_vals[a][i] for a in self.alternatives])
            for alt in self.alternatives:
                norm_vals[alt].append(lowest / alt_vals[alt][i] if alt_vals[alt][i] != 0 else 0)

        # Qualitative criteria 1–5 → 0–1
        for i in range(2, len(self.criteria)):
            for alt in self.alternatives:
                norm_vals[alt].append(min(alt_vals[alt][i],5)/5)

        # --- Weighted scores ---
        total_scores = {}
        for alt in self.alternatives:
            total = sum([norm_vals[alt][i] * weights[self.criteria[i]] for i in range(len(self.criteria))])
